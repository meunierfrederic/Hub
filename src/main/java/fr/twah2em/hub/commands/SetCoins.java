...
public class SetCoins implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args.length == 0 || args.length == 1) {
            ...
            sender.sendMessage("§cErreur: Vous devez préciser un joueur et me le montant d'argent que vous voulez lui définir.");
            return false;
        ...
        if (args.length == 2) {
            ...
            if (isNumeric(args[1])) {
                ...
                Hub.getInstance().getAccount(targetPlayer).get().getDataCoins().setCoins(Long.parseLong(args[1]));
                sender.sendMessage("§aVous avez bien fixé l'argent de §b" + targetPlayer.getName() + " §aà §6" + args[1] + "§2$§a.");
                targetPlayer.sendMessage("§b" + sender.getName() + " §aa fixé votre argent à §6" + args[1] + "§2$.");

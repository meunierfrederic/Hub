...
public class RemoveCoins implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args.length == 0 || args.length == 1) {
            ...
            sender.sendMessage("§cErreur: Vous devez préciser un joueur et me le montant d'argent que vous voulez lui enlever.");
            return false;
        ...
        if (args.length == 2) {
            ...
            if (isNumeric(args[1])) {
                ...
                Hub.getInstance().getAccount(targetPlayer).get().getDataCoins().removeCoins(Long.parseLong(args[1]));
                sender.sendMessage("§aVous avez bien retiré §6" + args[1] + "§2$ §aà §b" + targetPlayer.getName() + "§a.");
                targetPlayer.sendMessage("§b" + sender.getName() + " §cvous a enlevé §6" + args[1] + "§2$§a.");
